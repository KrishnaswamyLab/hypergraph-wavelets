...
def infer_cell_type(gene_matrix: sparse._csr.csr_matrix,
                    ...
                    gene_to_index: Dict,
                    threshold: float = 0.0) -> Tuple[sparse._csr.csr_matrix, List[str]]:
    '''
    ...
    if (final_assignments >= 0).all():
        cell_type_names = cell_types
        cell_type_matrix = cell_type_matrix[:, :-1]  # drop "Unassigned" column
    else:
        cell_type_names = cell_types + ['Unassigned']

    return sparse.csr_matrix(cell_type_matrix), cell_type_names, adata, final_assignments


def plot_overlay_images(adata,
                        final_assignments,
                        batch_name: str = '',
    ...
    if fig_spatial_save_path is not None:
        ...
        for label, color in zip(unique_labels, colors):
            ...
            ax.scatter(spatial_location['Y'][mask],
                       ...
                       facecolor=color, edgecolor='none', label=label, alpha=0.5, s=2)
        ax.set_xlabel('Spatial Y', fontsize=18)
        ax.set_ylabel('Spatial X', fontsize=18)
        ax.set_ylim(spatial_location['Y'].max(), 0)
        ax.set_title(batch_name, fontsize=20)
        ax.legend(fontsize=12, markerscale=5, bbox_to_anchor=(1.05, 1), loc='upper left')
        fig.tight_layout(pad=2)
        ...
        for opacity in [0.2, 0.5, 0.8]:
            ...
            for label, color in zip(unique_labels, colors):
                ...
                ax.scatter(spatial_location['pixel_col_in_highres'][mask],
                           ...
                           facecolor=color, edgecolor='none', label=label, alpha=opacity, s=2)
            ax.set_xlabel('Spatial Y', fontsize=18)
            ax.set_ylabel('Spatial X', fontsize=18)
        ...
        for label, color in zip(unique_labels, colors):
            ...
            ax.scatter(spatial_location['pixel_col_in_highres'][mask],
                       spatial_location['pixel_row_in_highres'][mask],
                       facecolor=color, edgecolor='none', label=label, alpha=0.8, s=2)
        ax.set_xlabel('Spatial Y', fontsize=18)
        ax.set_ylabel('Spatial X', fontsize=18)
        ax.set_ylim(spatial_location['pixel_row_in_highres'].max(), 0)
        ax.set_title(batch_name, fontsize=20)
...
def visualize_gene_expression_profiles(gene_matrix: sparse._csr.csr_matrix,
    ...
    return mean_expression_gene_by_celltype


def compute_gene_expression_by_celltype(gene_matrix: sparse._csr.csr_matrix,
                                        cell_type_matrix: sparse._csr.csr_matrix,
                                        cell_type_names: List[str]):
    """
    Parameters:
    ...
    for ct_idx, _ in enumerate(cell_type_names):
        cell_mask = cell_type_matrix[:, ct_idx].toarray().flatten().astype(bool)

        if cell_mask.sum() > 0:  # If there are cells of this type
            cell_expressions = normalized_matrix[cell_mask, :]
            mean_expressions = np.array(cell_expressions.mean(axis=0)).flatten()
            mean_expression_gene_by_celltype[:, ct_idx] = mean_expressions
...
def quantify_statistics(batch_index: str,
                        ...
                        mean_expression_gene_by_celltype: np.ndarray,
                        cell_type_names: List[str],
                        csv_path: str) -> None:
    '''
    ...
    if 'Unassigned' in cell_type_names:
        pixel_assigned_count = cell_type_counts[:-1].sum()
    else:
        pixel_assigned_count = cell_type_counts.sum()
    ...
    for cell_type, count in zip(cell_type_names, cell_type_counts):
        row_data[f'count_{cell_type}'] = count

    for ct_idx, cell_type in enumerate(cell_type_names):
        expr_count = mean_expression_gene_by_celltype.mean(0)[ct_idx]
        row_data[f'mean_expression_{cell_type}'] = expr_count
...
def save_all_expressions(batch_index: str,
                         disease_name: str,
                         mean_expression_gene_by_celltype: np.ndarray,
                         gene_names: List[str],
                         cell_type_names: List[str],
    ...
    if os.path.isfile(npz_path):
        ...
        assert cell_type_names == npz_dict['cell_type_names'].tolist()
        batch_index_list = npz_dict['batch_index'].tolist()
        batch_index_list.append(batch_index)
        disease_name_list = npz_dict['disease_name'].tolist()
        disease_name_list.append(disease_name)
        gene_by_celltype_matrices = npz_dict['gene_by_celltype_matrices']
        gene_by_celltype_matrices = np.concatenate((gene_by_celltype_matrices,
                                                    mean_expression_gene_by_celltype[None, ...]),
                                                   axis=0)
    else:
        batch_index_list = [batch_index]
        disease_name_list = [disease_name]
        gene_by_celltype_matrices = mean_expression_gene_by_celltype[None, ...]

    np.savez(npz_path,
             batch_index=np.array(batch_index_list),
             disease_name=np.array(disease_name_list),
             gene_names=np.array(gene_names),
             cell_type_names=np.array(cell_type_names),
             gene_by_celltype_matrices=gene_by_celltype_matrices)
    return
...
if __name__ == '__main__':
    ...
    for source_mat_folder, source_image_path, target_folder in tqdm(zip(filtered_folders, filtered_image_paths, filtered_folder_names),
        ...
        full_matrix = matrix.X.T
        full_gene_to_index = {gene_name: idx for idx, gene_name in enumerate(features[1])}
        ...
        celltype_related_matrix = full_matrix[:, celltype_related_feature_indices]
        ...
        full_matrix = full_matrix[barcode_position_in_image, :]
        pixel_count = barcode_position.shape[0]

        celltype_label_matrix, cell_type_names, adata, final_assignments = infer_cell_type(
        ...
        plot_overlay_images(
            ...
            fig_pc_save_path=f'./{dataset_name}/analysis_vis_celltype/{target_folder}_pc.png',
            fig_spatial_save_path=f'./{dataset_name}/analysis_vis_celltype/{target_folder}_spatial.png',
            spatial_location=barcode_position[['X', 'Y', 'pixel_row_in_highres', 'pixel_col_in_highres']],
            overlay_image=image)

        mean_expression_related_gene_by_celltype = visualize_gene_expression_profiles(
            ...
            batch_name=target_folder,
            save_path=f'./{dataset_name}/analysis_vis_celltype/{target_folder}_expression_heatmap.png'
        )

        mean_expression_all_gene_by_celltype = compute_gene_expression_by_celltype(
            gene_matrix=full_matrix,
            cell_type_matrix=celltype_label_matrix,
            cell_type_names=cell_type_names,
        )
        ...
        quantify_statistics(batch_index=batch_index,
                            ...
                            mean_expression_gene_by_celltype=mean_expression_related_gene_by_celltype,
                            cell_type_names=cell_type_names,
                            csv_path=f'./{dataset_name}/dataset_statistics.csv')

        save_all_expressions(batch_index=batch_index,
                             disease_name=disease_name,
                             mean_expression_gene_by_celltype=mean_expression_all_gene_by_celltype,
                             gene_names=features[1].to_numpy().tolist(),
                             cell_type_names=cell_type_names,
                             npz_path=f'./{dataset_name}/dataset_all_expressions.npz')
